...
class GameSceneProps
{
    ...
    public float WrongAnimationTime = 0.3f;
...
class GameScene : MonoBehaviour
{
    ...
    private void InitBindView()
    {
        view = new GameSceneView()
        {
            ...
            Badge0 = gameObject.FindChild("Badge0"),
            Badge1 = gameObject.FindChild("Badge1"),
        ...
        foreach (Movement move in Enum.GetValues(typeof(Movement)))
        {
            ...
            GameObject gg = GameObject.Find(name);
            gg.SetActive(false);
            gg.AddComponent<FadeAway>().callback = () => {
                gg.SetActive(false);
            };
...
//        Main.Theme.Apply(GameObject.Find("UICanvas"), true);
    ...
    private void InitAnimations()
    {
        ...
        foreach (Movement move in Enum.GetValues(typeof(Movement))) {
            string kk = (String.Format(Globals.GAMEOBJECT_MOVE, move, true));
            view.GameMoves[kk].AddComponent<BlinkAnimator>();
        }
        WinAnimation
    ...
    {
        GameObject go = view.GameWrong[move];
        go.SetActive(true);
        go.GetComponent<FadeAway>().Start();
    }
    ...
    {
        foreach (Movement move in Enum.GetValues(typeof(Movement))) {
            bool enabled = this.Model.AvailableMoves.Contains(move);
            view.GameMoves[String.Format(Globals.GAMEOBJECT_MOVE, move, true)].GetComponent<SpriteRenderer>().enabled = enabled;
            view.GameMoves[String.Format(Globals.GAMEOBJECT_MOVE, move, false)].GetComponent<SpriteRenderer>().enabled  =!enabled;
        }
    }
    ...
    {
        bool enabled = Model.CanUndo;
        view.UndoButton.GetComponent<Button>().interactable = enabled;
        view.Badge1.SetActive(enabled);
        view.Badge0.SetActive(!enabled);
    }
